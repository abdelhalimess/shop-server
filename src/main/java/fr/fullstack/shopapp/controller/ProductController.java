...
public class ProductController {
    ...
    public ResponseEntity<Page<Product>> getProductsOfShop(
            @ParameterObject Pageable pageable, // Remplace @ApiImplicitParams
            @Parameter(description = "Id of the shop", example = "1") @RequestParam(required = false) Optional<Long> shopId,
            @Parameter(description = "Id of the category", example = "1") @RequestParam(required = false) Optional<Long> categoryId
    ) {
    ...
    public ResponseEntity<Page<Product>> searchProducts(
        ...
        return ResponseEntity.ok(service.searchProduct(input, pageable));
    }
}